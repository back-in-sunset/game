import { encodeAuthFrame, encodeHeartbeat, encodeSendFrame, decodeFrame } from "@game/api";
import type { IMAuthRequest, IMClientStatus } from "@game/api";
...
export class IMClient {
  ...
  private sendPacket(frame: Uint8Array): void {
  ...
  private async doConnect(auth: IMAuthRequest): Promise<void> {
    ...
    await new Promise<void>((resolve, reject) => {
      if (!this.socket) { reject(new Error("socket not created")); return; }
  ...
  private onMessage = async (event: WebSocketMessageEvent): Promise<void> => {
    if (typeof event.data === "string") return;
    const buffer = await (event.data as Blob).arrayBuffer();
    const frame = decodeFrame(buffer);
    if (frame.op === 3) return; // heartbeat reply
  ...
  private onClose = (): void => {
    ...
    this.reconnectTimer = setTimeout(() => {
      if (this.auth) { this.doConnect(this.auth).catch(() => this.onClose()); }
    }, delay);
  };
}
