...
func (l *BlockLogic) Block(req *types.CommentActionRequest) (*types.CommentActionResponse, error) {
	if req.ObjID <= 0 {
	...
	res, err := l.svcCtx.CommentRpc.BlockComment(l.ctx, &commentclient.BlockCommentRequest{
		ObjID:     req.ObjID,
		ObjType:   req.ObjType,
