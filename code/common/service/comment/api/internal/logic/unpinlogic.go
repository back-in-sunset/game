...
func (l *UnpinLogic) Unpin(req *types.CommentActionRequest) (*types.CommentActionResponse, error) {
	if req.ObjID <= 0 {
	...
	res, err := l.svcCtx.CommentRpc.UnSetCommentAttrs(l.ctx, &commentclient.UnSetCommentAttrsRequest{
		ObjID:     req.ObjID,
		ObjType:   req.ObjType,
