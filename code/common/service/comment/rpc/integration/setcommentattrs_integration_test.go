...
func TestSetCommentAttrs_Integration(t *testing.T) {
	if testing.Short() {
		t.Skip("skip integration test in short mode")
	}

	ctx, dsn := testkit.StartMySQLContainer(t, "comment_pin")
	db := testkit.OpenMySQLWithRetry(t, ctx, dsn)

	testObjIDs := []int64{300001, 300002, 300003, 300004, 300005, 300006}
...
CREATE TABLE IF NOT EXISTS comment_content_%d (
  ...
  KEY idx_comment_obj_unique (comment_id, obj_id)
	...
	t.Run("pin_comment_successfully", func(t *testing.T) {
		objID := int64(300001)
		objType := int64(1)
		memberID := int64(5001)
		...
		addResp, err := addLogic.AddComment(&comment.CommentRequest{
			...
			ObjType:  objType,
			MemberID: memberID,
			Message:  "test comment for pinning",
		})
		if err != nil {
		...
		pinResp, err := pinLogic.SetCommentAttrs(&comment.SetCommentAttrsRequest{
			ObjID:     objID,
			ObjType:   objType,
		...
		if pinResp.Message != "ok" {
			t.Errorf("expected message='ok', got %v", pinResp.Message)
		}
		...
		if pinnedComment.Attrs != 1 {
			t.Errorf("expected attrs=1 (pinned), got %d", pinnedComment.Attrs)
		}
	})

	t.Run("pin_already_pinned_comment", func(t *testing.T) {
		objID := int64(300002)
		objType := int64(1)
		memberID := int64(5002)
		...
		addResp, err := addLogic.AddComment(&comment.CommentRequest{
			...
			ObjType:  objType,
			MemberID: memberID,
			Message:  "test comment for double pinning",
		})
		if err != nil {
			t.Fatalf("AddComment() error = %v", err)
		}
		...
		if err != nil {
			t.Fatalf("First SetCommentAttrs() error = %v", err)
		}

		secondPinResp, err := pinLogic.SetCommentAttrs(&comment.SetCommentAttrsRequest{
			ObjID:     objID,
			ObjType:   objType,
			CommentID: addResp.CommentID,
			MemberID:  memberID,
		...
		if !secondPinResp.Success {
			t.Errorf("expected success=true for second pin, got %v", secondPinResp.Success)
		}

		stillPinnedComment, err := commentModel.FindOneByObjID(context.Background(), objID, addResp.CommentID)
		if err != nil {
			t.Fatalf("FindOneByObjID() error = %v", err)
		}
		if stillPinnedComment.Attrs != 1 {
			t.Errorf("expected attrs=1 (still pinned), got %d", stillPinnedComment.Attrs)
		}
	})

	t.Run("pin_nonexistent_comment", func(t *testing.T) {
		objID := int64(300003)
		objType := int64(1)
		memberID := int64(5003)
	...
	t.Run("pin_multiple_comments", func(t *testing.T) {
		objID := int64(300004)
		objType := int64(1)
		memberID := int64(5004)
		...
		for _, commentID := range comments {
			resp, err := pinLogic.SetCommentAttrs(&comment.SetCommentAttrsRequest{
				ObjID:     objID,
				ObjType:   objType,
				CommentID: commentID,
				MemberID:  memberID,
		...
		for _, commentID := range comments {
			...
			if commentData.Attrs != 1 {
				t.Errorf("expected attrs=1 (pinned), got %d for comment %d", commentData.Attrs, commentID)
			}
		}
	})

	t.Run("pin_comment_validation", func(t *testing.T) {
		objID := int64(300005)
		objType := int64(1)
