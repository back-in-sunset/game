...
func TestBlockComment_Integration(t *testing.T) {
	if testing.Short() {
		t.Skip("skip integration test in short mode")
	}

	ctx, dsn := testkit.StartMySQLContainer(t, "comment_block")
	db := testkit.OpenMySQLWithRetry(t, ctx, dsn)

	testObjIDs := []int64{100001, 100002, 100003, 100004, 100005}
...
CREATE TABLE IF NOT EXISTS comment_content_%d (
  ...
  KEY idx_comment_obj_unique (comment_id, obj_id)
	...
	t.Run("block_comment_successfully", func(t *testing.T) {
		objID := int64(100001)
		objType := int64(1)
		memberID := int64(3001)
		...
		addResp, err := addLogic.AddComment(&comment.CommentRequest{
			...
			ObjType:  objType,
			MemberID: memberID,
			Message:  "test comment for blocking",
		})
		if err != nil {
		...
		blockResp, err := blockLogic.BlockComment(&comment.BlockCommentRequest{
			ObjID:     objID,
			ObjType:   objType,
		...
		if blockedComment.State != 1 {
			t.Errorf("expected state=1 (blocked), got %d", blockedComment.State)
		}
	})

	t.Run("block_already_blocked_comment", func(t *testing.T) {
		objID := int64(100002)
		objType := int64(1)
		memberID := int64(3002)
		...
		addResp, err := addLogic.AddComment(&comment.CommentRequest{
			...
			ObjType:  objType,
			MemberID: memberID,
			Message:  "test comment for double blocking",
		})
		if err != nil {
			t.Fatalf("AddComment() error = %v", err)
		}
		...
		if err != nil {
			t.Fatalf("First BlockComment() error = %v", err)
		}

		secondBlockResp, err := blockLogic.BlockComment(&comment.BlockCommentRequest{
			ObjID:     objID,
			ObjType:   objType,
			CommentID: addResp.CommentID,
			MemberID:  memberID,
		...
		if !secondBlockResp.Success {
			t.Errorf("expected success=true for second block, got %v", secondBlockResp.Success)
		}
	...
	t.Run("block_nonexistent_comment", func(t *testing.T) {
		objID := int64(100003)
		objType := int64(1)
		memberID := int64(3003)
	...
	t.Run("block_multiple_comments", func(t *testing.T) {
		objID := int64(100004)
		objType := int64(1)
		memberID := int64(3004)
		...
		for _, commentID := range comments {
			resp, err := blockLogic.BlockComment(&comment.BlockCommentRequest{
				ObjID:     objID,
				ObjType:   objType,
				CommentID: commentID,
				MemberID:  memberID,
		...
		for _, commentID := range comments {
			...
			if commentData.State != 1 {
				t.Errorf("expected state=1 (blocked), got %d for comment %d", commentData.State, commentID)
			}
		}
	})

	t.Run("block_comment_validation", func(t *testing.T) {
		objID := int64(100005)
		objType := int64(1)
